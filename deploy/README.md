...
A interface ficará disponível na porta definida por `PAC_HTTP_PORT` (padrão `8080`). Para exposição pública, use domínio e proxy HTTPS institucional; não exponha a aplicação sem controle de acesso enquanto a autenticação OIDC não estiver conectada.
